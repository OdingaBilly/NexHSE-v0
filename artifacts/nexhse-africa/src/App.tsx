...
const meta: Record<string, { title: string; description: string }> = {
  home: { title: 'NexHSE Africa | Workplace Safety, HSE Training & Professional Development', description: 'NexHSE Africa helps organisations build safer, smarter and more compliant workplaces through HSE audits, risk assessment, fire safety, training and environmental services.' },
...
function Seo({ page = 'home', title, description }: { page?: string; title?: string; description?: string }) {
  useEffect(() => {
    ...
    set('description', finalDescription); set('twitter:card', 'summary_large_image'); set('twitter:title', finalTitle); set('twitter:description', finalDescription);
    setProperty('og:title', finalTitle); setProperty('og:description', finalDescription); setProperty('og:type', 'website'); setProperty('og:url', `https://nexhse.co.ke${window.location.pathname}`);
    let canonical = document.querySelector('link[rel="canonical"]') as HTMLLinkElement | null;
    if (!canonical) { canonical = document.createElement('link'); canonical.rel = 'canonical'; document.head.appendChild(canonical); }
    canonical.href = `https://nexhse.co.ke${window.location.pathname}`;
  ...
  return <header className="relative z-40 border-b border-[hsl(var(--border)/.7)] bg-[hsl(var(--background)/.93)] backdrop-blur-md">
    <div className="mx-auto flex h-[76px] max-w-7xl items-center justify-between px-5 lg:px-8">
      ...
      <nav className="hidden items-center gap-7 lg:flex" aria-label="Primary navigation">
        {links.map(([label, href]) => <Link key={href} href={href} className="focus-ring text-[13px] font-semibold text-[hsl(var(--muted-foreground))] transition-colors hover:text-[hsl(var(--primary))]" data-testid={`link-nav-${label.toLowerCase()}`}>{label}</Link>)}
      </nav>
...
function TrustStrip() {
  return <section className="border-b border-[hsl(var(--border))] bg-[hsl(var(--card))]"><div className="mx-auto max-w-7xl px-5 py-8 lg:px-8"><div className="grid gap-6 sm:grid-cols-3 lg:grid-cols-5">{[['01', 'Content Required', 'Content Required'], ['02', 'Content Required', 'Content Required'], ['03', 'Content Required', 'Content Required'], ['04', 'Content Required', 'Content Required'], ['05', 'Content Required', 'Content Required']].map(([n, label, value]) => <div key={n} className="flex items-start gap-3 border-l-2 border-[hsl(var(--accent)/.4)] pl-4"><span className="mono-label text-[10px] text-[hsl(var(--accent))]">{n}</span><div><p className="text-[11px] font-bold uppercase tracking-wide text-[hsl(var(--muted-foreground))]">{label}</p><p className="mt-1 text-xs text-[hsl(var(--primary))]">{value}</p></div></div>)}</div></div></section>;
}

function ContentRequiredSection({ eyebrow = 'Content Required', title = 'Content Required', text = 'Content Required', image }: { eyebrow?: string; title?: string; text?: string; image?: string }) {
  return <section className="content-required-section relative overflow-hidden px-5 py-20 lg:px-8 lg:py-24"><div className="content-required-outline content-required-outline--one" aria-hidden="true" /><div className="content-required-outline content-required-outline--two" aria-hidden="true" /><div className="relative z-10 mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-[.9fr_1.1fr]"><div><p className="mono-label text-[10px] font-bold text-[hsl(var(--accent))]">{eyebrow}</p><h2 className="display mt-4 max-w-2xl text-5xl leading-[1.03] tracking-[-.05em] text-[hsl(var(--primary))] sm:text-6xl">{title}</h2><p className="mt-5 max-w-xl text-base leading-8 text-[hsl(var(--muted-foreground))]">{text}</p></div>{image ? <OrganicImage src={image} alt="" className="content-required-image relative h-56 lg:h-72" /> : <div className="content-required-card rounded-[2rem] border border-[hsl(var(--border))] bg-[hsl(var(--card)/.76)] p-7 backdrop-blur-sm"><span className="grid h-12 w-12 place-items-center rounded-full bg-[hsl(var(--secondary))] text-[hsl(var(--accent))]"><Sparkles size={19} /></span><p className="mono-label mt-10 text-[10px] text-[hsl(var(--accent))]">Content Required</p><p className="mt-3 max-w-sm text-sm leading-6 text-[hsl(var(--muted-foreground))]">Content Required</p></div>}</div></section>;
}
...
function Home() {
  return <Shell><Seo /><main className="home-page">
    ...
    <TrustStrip />
     <ContentRequiredSection eyebrow="Homepage section" title="Content Required" text="Content Required" />
     <ContentRequiredSection eyebrow="Homepage section" title="Content Required" text="Content Required" image={trainingImage} />
      <section className="home-projects-section relative overflow-hidden px-5 py-16 lg:px-8 lg:py-20">
       ...
       <div className="relative z-10 mx-auto max-w-7xl">
         <div className="home-projects-header mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
           <div>
              <p className="mono-label text-[10px] font-bold text-[hsl(var(--primary))]">Content Required</p>
              <h2 className="display mt-3 max-w-2xl text-4xl leading-[1.08] tracking-[-.04em] text-[hsl(var(--primary))] sm:text-5xl">Content Required</h2>
           </div>
            <p className="max-w-xl text-sm leading-7 text-[hsl(var(--primary)/.72)]">Content Required</p>
         </div>
         <div className="home-projects-grid grid gap-5 lg:grid-cols-[1.35fr_.65fr]">
           <div className="home-projects-feature relative min-h-[370px] overflow-hidden">
             ...
             <div className="home-projects-feature-copy">
                <span className="mono-label text-[10px] text-[hsl(var(--secondary))]">Content Required</span>
                <h3 className="display mt-3 max-w-md text-3xl text-white">Content Required</h3>
                <p className="mt-3 max-w-md text-sm text-white/70">Content Required</p>
             </div>
           </div>
           <div className="home-projects-side flex min-h-[370px] flex-col justify-between">
             <div>
               <BriefcaseBusiness className="home-projects-side-icon" />
                <p className="mono-label mt-14 text-[10px] font-bold text-[hsl(var(--primary))]">Content Required</p>
                <h3 className="mt-3 text-2xl font-bold text-[hsl(var(--primary))]">Content Required</h3>
                <p className="mt-3 max-w-xs text-sm leading-6 text-[hsl(var(--primary)/.72)]">Content Required</p>
             </div>
