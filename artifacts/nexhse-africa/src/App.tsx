...
function Seo({ page = 'home', title, description }: { page?: string; title?: string; description?: string }) {
  useEffect(() => {
    ...
    setProperty('og:title', finalTitle); setProperty('og:description', finalDescription); setProperty('og:type', (page === 'knowledge' || page === 'blog') && title ? 'article' : 'website'); setProperty('og:url', canonicalUrl); setProperty('og:site_name', 'NexHSE Africa'); setProperty('og:locale', 'en_KE'); setProperty('og:image', 'https://nexhse.co.ke/logo.png');
    ...
    structuredData.textContent = JSON.stringify({
      ...
      '@graph': [
        ...
        ...((page === 'knowledge' || page === 'blog') && title ? [{ '@type': 'Article', headline: finalTitle, description: finalDescription, url: canonicalUrl, author: { '@type': 'Organization', name: 'NexHSE Africa', url: 'https://nexhse.co.ke/' }, publisher: { '@id': 'https://nexhse.co.ke/#organization' }, mainEntityOfPage: { '@type': 'WebPage', '@id': `${canonicalUrl}#webpage` } }] : []),
      ],
    });
...
function Navbar() {
  ...
  const links = [['About', '/about'], ['Services', '/services'], ['Training', '/training'], ['Knowledge', '/knowledge'], ['Blog', '/blog'], ['Contact', '/contact']];
  useEffect(() => {
    if (!open) return;
...
function Shell({ children }: { children: ReactNode }) {
  return <div className="grain min-h-[100dvh]"><Navbar /><PageCanvasArtwork />{children}<Footer /><MobileActions /></div>;
}
